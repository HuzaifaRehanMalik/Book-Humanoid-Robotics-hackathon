...
const SidebarChat = ({ isOpen, onClose }) => {
    ...
    if (!isOpen)
        return null;
    return (<div className={styles['sidebar-chat-overlay']}>
      <div className={styles['sidebar-chat']}>
        <div className={styles['sidebar-chat-header']}>
          <h3>Textbook Assistant</h3>
          <button className={styles['sidebar-chat-close-button']} onClick={onClose} aria-label="Close chat">
            ×
          </button>
        </div>

        <div className={styles['sidebar-chat-messages']}>
          {messages.length === 0 ? (<div className={styles['sidebar-chat-welcome']}>
            ...
            </div>) : (messages.map((message) => (<div key={message.id} className={`${styles['sidebar-chat-message']} ${styles[`sidebar-chat-message--${message.role}`]}`}>
                <div className={styles['sidebar-chat-message-content']}>
                  {message.content}
                  {message.sources?.length ? (<div className={styles['sidebar-chat-message-sources']}>
        ...
        <form onSubmit={handleSubmit} className={styles['sidebar-chat-input-form']}>
          <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} placeholder="Ask a question about the textbook..." disabled={isLoading} className={styles['sidebar-chat-input']}/>
          <button type="submit" disabled={isLoading || !inputValue.trim()} className={styles['sidebar-chat-send-button']}>
            {isLoading ? '...' : '→'}
          </button>
        </form>
      </div>
